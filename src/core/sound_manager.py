...
class SoundManager:
    ...
    async def play_sound(self, sound_file: str):
        ...
        try:
            ...
            logger.info(f"Playing sound: {sound_path} at volume {settings.NOTIFICATION_VOLUME}%")
    ...
    async def notify(self, event: SystemEvent):
        ...
        try:
            if event in self.event_sounds:
                ...
                logger.info(f"Playing notification for event: {event.value} using {sound_file}")
